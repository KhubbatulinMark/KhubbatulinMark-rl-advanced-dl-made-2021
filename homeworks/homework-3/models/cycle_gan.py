import torch
import itertools
from .base import BaseModel, ImagePool
...
class CycleGAN(BaseModel):
    ...
    def __init__(self, model_name, params, device='cuda'):
        ...
        if self.phase == 'train':
            self.netD_A = define_D(
                3, 64, 'basic', 3, 'instance', 'normal',  0.02, [0]
            )
        ...
        if self.phase == 'train':
            ...
            self.optimizer_G = torch.optim.Adam(
                itertools.chain(
                    self.netG_A.parameters(), self.netG_B.parameters()), lr=params['lr'], betas=(params['beta'], 0.999)
            )
            self.optimizer_D = torch.optim.Adam(
                itertools.chain(
                    self.netD_A.parameters(), self.netD_B.parameters()), lr=params['lr'], betas=(params['beta'], 0.999)
            )
    ...
    def backward_G(self):
        ...
        self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * 10 * 5
        # G_B should be identity if real_A is fed: ||G_B(A) - A||
        self.idt_B = self.netG_B(self.real_A)
        self.loss_idt_B = self.criterionIdt(self.idt_B, self.real_A) * 10 * 5
